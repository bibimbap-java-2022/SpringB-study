...
public class Main {
	...
	public static void main(String[] args) {
		// 학번,이름 출력
		System.out.println("202211351 이한샘");
	...
	private static Boolean checkBingo(int map[][]) {
		...
		for (int i = 0; i < ROW; i++) { // 우하향 대각선 빙고 확인
			isZero = true;
			if (map[i][i] != 0) {
				isZero = false;
			}
		...
		for (int i = 0; i < ROW; i++) { // 우상향 대각선 빙고 확인
			isZero = true;
			if (map[i][4 - i] != 0) {
				isZero = false;
			}
